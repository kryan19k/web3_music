...
// Use the new CollectionMusicNFT contract address
const COLLECTION_MUSIC_NFT_ADDRESS = CONTRACTS.CollectionMusicNFT.address
...
export function useAdminRoleInfo(address?: Address) {
  ...
  if (!isDeployer && (defaultAdminRole.error || managerRole.error || artistRole.error)) {
    console.log('Admin Role Info Errors:', {
      contractAddress: COLLECTION_MUSIC_NFT_ADDRESS,
...
export function useAdminGrantRole() {
  ...
  const grantRole = useMutation({
    mutationFn: async ({ role, account }: { role: string, account: Address }) => {
      console.log('🔵 Grant role called with:', { role, account, contractAddress: COLLECTION_MUSIC_NFT_ADDRESS })
